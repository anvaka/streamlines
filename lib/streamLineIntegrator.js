...
function createStreamlineIntegrator(start, grid, config) {
  ...
  var maximumPointsPerLine = false;

  if (config.maximumPointsPerLine) {
    if (typeof config.maximumPointsPerLine === 'number') {
      maximumPointsPerLine = config.maximumPointsPerLine;
    } else {
      maximumPointsPerLine = config.maximumPointsPerLine();
    }
  }
  ...
  function next() {
    while(true) {
      candidate = null;
      if (maximumPointsPerLine && points.length > maximumPointsPerLine) {
        state = DONE;
      } 
      if (state === FORWARD) {
